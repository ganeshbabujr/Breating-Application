...
function initHistory() {
    ...
    document.querySelectorAll('.tab').forEach(t => {
        t.addEventListener('click', (e) => {
            ...
            if (currentView !== newView) {
                currentView = newView; dateOffset = 0; renderDashboard();
            }
        });
    });
    ...
    document.getElementById('clearHistoryBtn').onclick = () => {
        if(confirm("Are you sure you want to clear ALL practice history?")) {
            localStorage.removeItem('pranaHistory');
            renderDashboard();
        }
    };
...
function exportData() {
    const data = {
        history: JSON.parse(localStorage.getItem('pranaHistory') || '[]'),
        presets: JSON.parse(localStorage.getItem('pranaPresets') || '[]'),
        theme: localStorage.getItem('pranaTheme') || 'dark'
    };
...
function importData(input) {
    ...
    reader.onload = function(e) {
        try {
            ...
            if (data.history) localStorage.setItem('pranaHistory', JSON.stringify(data.history));
            if (data.presets) localStorage.setItem('pranaPresets', JSON.stringify(data.presets));
            if (data.theme) localStorage.setItem('pranaTheme', data.theme);
            alert("Data restored successfully! Page will reload.");
            location.reload();
        } catch (err) {
            alert("Error reading file. Make sure it is a valid backup.");
        }
    };
...
function renderDashboard() {
    ...
    // --- GAMIFICATION LOGIC (SANSKRIT MONTHLY CHALLENGE) ---
    const now = new Date();
    const currentMonthIdx = now.getMonth();
    const currentYear = now.getFullYear();
    const daysInMonth = new Date(currentYear, currentMonthIdx + 1, 0).getDate();
    
    // 1. Filter History for THIS MONTH ONLY
    const monthlySessions = history.filter(s => {
        const d = new Date(s.date);
        return d.getMonth() === currentMonthIdx && d.getFullYear() === currentYear;
    });

    // 2. Calculate Total Minutes This Month
    ...
    // 3. Define Targets based on your 50min/day goal
    const monthlyGoal = 50 * daysInMonth; 
    
    // SANSKRIT LEVELS
    const levels = [
        { pct: 0,   title: "Arambha",      icon: "🌱" }, // Novice
        { pct: 25,  title: "Sadhaka",      icon: "🌿" }, // Practitioner
        { pct: 50,  title: "Abhyasi",      icon: "🧘" }, // Diligent
        { pct: 75,  title: "Sthira",       icon: "🔥" }, // Steady
        { pct: 100, title: "Siddha",       icon: "👑" }, // Master
        { pct: 120, title: "Mahayogi",     icon: "✨" }  // Grandmaster
    ];

    // 4. Determine Current Level
    ...
    for (let i = 0; i < levels.length; i++) {
        if (currentPct >= levels[i].pct) {
            currentLevel = levels[i];
            nextLevel = levels[i+1] || null;
        }
    }

    // 5. Update UI (New Zen-Glass Elements)
    ...
    if (nextLevel) {
        ...
        // Bar fills relative to current tier
    ...
    // Filter Data
    const filteredData = history.filter(s => {
        const d = new Date(s.date);
    ...
    filteredData.forEach(s => {
        ...
        if (index >= 0 && index < dataPoints.length) {
            dataPoints[index] += (s.duration / 60); 
        }
    });
...
function renderDetailedLog(history) {
    const listEl = document.getElementById('detailedLogList');
    if (!listEl) return;
    listEl.innerHTML = '';
...
// --- MISSED DAYS / VISUAL HELPER ---
function renderMissedDays(startDate, endDate, history) {
    const listEl = document.getElementById('missedDaysList');
    const calEl = document.getElementById('calendarViewContainer');
    const legendEl = document.getElementById('calendarLegend');
    const badgeEl = document.getElementById('missedCountBadge');
    const titleEl = document.getElementById('missedCardTitle');
    
    if (!listEl || !calEl) return;

    const practicedDates = new Set(history.map(s => {
        const d = new Date(s.date);
    ...
    if (currentView === 'month') {
        listEl.style.display = 'none'; badgeEl.style.display = 'none';
        calEl.style.display = 'block'; legendEl.style.display = 'flex';
        titleEl.textContent = "Monthly Overview"; titleEl.style.color = "var(--text-main)";
        renderCalendarGrid(calEl, startDate, practicedDates);
    } 
    // --- WEEK VIEW ---
    else if (currentView === 'week') {
        calEl.style.display = 'none'; legendEl.style.display = 'none';
        listEl.style.display = 'flex'; listEl.style.flexDirection = 'column';
        badgeEl.style.display = 'none';
        titleEl.textContent = "Weekly Overview"; titleEl.style.color = "var(--text-main)";

        const days = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];
        let html = '<div class="week-view-container"><div class="week-strip">';
        
        let activeDays = 0; let totalWeekMins = 0;
        const today = new Date(); today.setHours(0,0,0,0);
        let loopDay = new Date(startDate);
        
        for (let i = 0; i < 7; i++) {
            const dateStr = loopDay.toLocaleDateString('en-CA');
            const dayLabel = days[i];
            const daySessions = history.filter(s => {
                const d = new Date(s.date);
                return d.toLocaleDateString('en-CA') === dateStr;
            });
            const isPracticed = daySessions.length > 0;
            const isFuture = loopDay > today;
            
            let cardClass = ''; let icon = ''; let val = '';
            
            if (isPracticed) {
                activeDays++;
                const mins = Math.round(daySessions.reduce((acc, s) => acc + (s.duration || 0), 0) / 60);
                totalWeekMins += mins;
                cardClass = 'done'; icon = '✓'; val = `${mins}m`;
            } else if (isFuture) {
                cardClass = 'future'; icon = '-'; val = '--';
            } else {
                const isToday = loopDay.toDateString() === today.toDateString();
                if (isToday) { cardClass = 'future'; icon = '⏳'; val = 'Pending'; }
                else { cardClass = 'missed'; icon = '✕'; val = 'Missed'; }
            }
            html += `<div class="week-day-card ${cardClass}"><span class="wd-label">${dayLabel}</span><span class="wd-icon">${icon}</span><span class="wd-val">${val}</span></div>`;
            loopDay.setDate(loopDay.getDate() + 1);
        }
        html += '</div>';
        const consistency = Math.round((activeDays / 7) * 100);
        html += `<div class="week-summary-row"><div class="ws-item"><div class="ws-val">${activeDays}/7</div><div class="ws-lbl">Days Active</div></div><div class="ws-item"><div class="ws-val">${totalWeekMins}m</div><div class="ws-lbl">Total Time</div></div><div class="ws-item"><div class="ws-val" style="color: ${consistency > 70 ? 'var(--cal-success)' : 'var(--text-muted)'}">${consistency}%</div><div class="ws-lbl">Score</div></div></div></div>`;
    ...
    // --- DAY VIEW ---
         ...
         listEl.style.display = 'flex'; listEl.style.flexDirection = 'column';
         badgeEl.style.display = 'none';
         titleEl.textContent = "Daily Targets"; titleEl.style.color = "var(--text-main)";
         ...
         let morningDone = false; let nightDone = false;   
         daySessions.forEach(s => {
             const h = new Date(s.date).getHours();
             if (h >= 4 && h < 12) morningDone = true;
             if (h >= 18) nightDone = true;
         });
         
         const now = new Date();
         const isToday = startDate.toDateString() === now.toDateString();
         const todayMidnight = new Date(now.getFullYear(), now.getMonth(), now.getDate());
         const viewDateMidnight = new Date(startDate.getFullYear(), startDate.getMonth(), startDate.getDate());
         const isPast = viewDateMidnight < todayMidnight;

         let mStatus = 'Pending'; let mClass = ''; let mCheck = '';
         if (morningDone) { mStatus = 'Completed'; mClass = 'done'; mCheck = '✓ '; }
         else { if (isPast || (isToday && now.getHours() >= 12)) { mStatus = 'Missed'; mClass = 'missed'; mCheck = '✕ '; } }

         let nStatus = 'Pending'; let nClass = ''; let nCheck = '';
         if (nightDone) { nStatus = 'Completed'; nClass = 'done'; nCheck = '✓ '; }
         else { if (isPast) { nStatus = 'Missed'; nClass = 'missed'; nCheck = '✕ '; } }

         const renderCard = (statusClass, label, icon, statusText, check) => {
            return `<div class="target-card ${statusClass}"><div class="target-icon">${icon}</div><div class="target-label">${label}</div><div class="target-status">${check}${statusText}</div></div>`;
         };

         const totalCycles = daySessions.reduce((acc, s) => acc + (s.cycles || 0), 0);
         const totalMins = Math.round(daySessions.reduce((acc, s) => acc + (s.duration || 0), 0) / 60);
    ...
    // --- YEAR VIEW ---
         ...
         listEl.style.display = 'flex'; listEl.style.flexDirection = 'column';
         badgeEl.style.display = 'none';
         titleEl.textContent = "Yearly Performance"; titleEl.style.color = "var(--text-main)";
         ...
         const sortedDates = [];
         practicedDates.forEach(d => { if(d.startsWith(year.toString())) { activeDaysCount++; sortedDates.push(d); } });
         sortedDates.sort();
         ...
         if (sortedDates.length > 0) {
            sortedDates.forEach(dateStr => {
                ...
                if (prevDate) {
                    ...
                    if (diffDays === 1) currStreak++; else currStreak = 1;
                } else { currStreak = 1; }
         ...
         if (year < currentYear) totalDaysToCount = daysInYear;
         else if (year === currentYear) {
             const startOfYear = new Date(year, 0, 0);
             const diff = now - startOfYear;
             const oneDay = 1000 * 60 * 60 * 24;
             totalDaysToCount = Math.floor(diff / oneDay);
         } else totalDaysToCount = 0;
         
         const missedCount = Math.max(0, totalDaysToCount - activeDaysCount);
         const completionRate = totalDaysToCount > 0 ? Math.round((activeDaysCount / totalDaysToCount) * 100) : 0;
...
function renderCalendarGrid(container, dateInMonth, practicedSet) {
    ...
    for (let dayNum = 1; dayNum <= daysInMonth; dayNum++) {
        ...
        if (isToday) classes += ' today-indicator';
...
function getDateRangeAndBuckets(view, offset) {
    ...
    if (view === 'day') {
        ...
        labels = ['Early', 'Morn', 'Noon', 'Aft', 'Eve', 'Night'];
    } else if (view === 'week') {
        const day = now.getDay(); const diff = now.getDate() - day + (day == 0 ? -6 : 1); 
        start.setDate(diff + (offset * 7)); start.setHours(0,0,0,0);
        end = new Date(start); end.setDate(start.getDate() + 6); end.setHours(23,59,59,999);
        labels = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];
    } else if (view === 'month') {
        start.setDate(1); start.setMonth(now.getMonth() + offset); start.setHours(0,0,0,0);
        ...
        start.setFullYear(now.getFullYear() + offset); start.setMonth(0, 1); start.setHours(0,0,0,0);
        end = new Date(start); end.setFullYear(start.getFullYear() + 1); end.setDate(0); end.setHours(23,59,59,999);
...
function renderBarChart(labels, data) {
    ...
    const accentColor = isLight ? '#06b6d4' : '#22d3ee'; 
    const gridColor   = isLight ? 'rgba(0,0,0,0.05)' : 'rgba(255,255,255,0.05)';
    const tickColor   = isLight ? '#64748b' : '#94a3b8';
    const barThickness = labels.length > 15 ? 6 : 12;
    ...
    practiceChartInstance = new Chart(ctx, {
        ...
        data: {
            labels: labels,
            datasets: [{
                data: data, backgroundColor: accentColor, borderRadius: 50, barThickness: barThickness, borderSkipped: false
            }]
        },
        options: {
            ...
            scales: {
                y: {
                    ...
                    border: { display: false }, grid: { color: gridColor, drawTicks: false },
                    ticks: { color: tickColor, font: { size: 10, family: "'Inter', sans-serif" }, padding: 10, callback: function(val) { return val + 'm'; } }
                },
                x: {
                    grid: { display: false }, ticks: { color: tickColor, font: { size: 10, family: "'Inter', sans-serif" }, autoSkip: true, maxTicksLimit: 15 }
                }
            }
        }
    });
